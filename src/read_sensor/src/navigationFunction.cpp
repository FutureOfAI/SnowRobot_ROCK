...
{
    double updatRate; // Integrated location and Navigation update rate [Hz]

    double dtRTK; // delta time from last RTK data update to now [Hz]

    bool baseType; // true is available, false is invalid
    POINT_XYZ baseBLH;
    POINT_XYZ baseENU; // origin point , coordinates is (0,0,0)
    POINT_XYZ baseECEF;

    bool rtkType;  // true is available, false is invalid
    double lostRtk; // the time of lost RTK data
    POINT_XYZ rtkBLH;
    POINT_XYZ rtkENU;
    POINT_XYZ rtkECEF;
    ...
    bool updateBest; // true is available, false is invalid
    POINT_XYZ bestPos; // in ECEF frame [m]
    POINT_XYZ bestVel; // in ECEF frame [m/s]

    // measure standard deviation error in ECEF frame
    POINT_XYZ stdPos; // position [m]
    POINT_XYZ stdVel; // velocity [m/s]

    // evaluation variance in navigation frame [rad/s]
    double varAtt; // attitude [rad/s]
    double varVel; // velocity [m/s]
    double varPos; // position [m]
    double varGyro; // gyro [m]
    double varAcc; // Accelerometer [m/s^2]

    double dtIMU;
    bool updateIMU;
    POINT_XYZ initialAngle;
    POINT_XYZ eulerAngle; // sensor Angle
    POINT_XYZ accel;
    POINT_XYZ gyro;
    POINT_XYZ a_last;
    POINT_XYZ g_last;
	...
	double gyroTau; // gyro bias correlation time [sec]
	double accelTau; // acc bias correlation time [sec]

    double dtOdom;
    POINT_XYZ wheelVel;
    POINT_XYZ wheelOdom;

    POINT_XYZ rtkPos; // the RTK install position relative to rear wheel centre
    POINT_XYZ imuPos; // the IMU install position relative to rear wheel centre
    POINT_XYZ armPos; // the IMU and RTK relative install position in body frame
...
typedef struct solutionType
{
    ...
    POINT_XYZ imuAttitude;
    POINT_XYZ imuVelocity;
    POINT_XYZ imuPosition; // in BLH coordinates frame

    POINT_XYZ robotAttitude;
    POINT_XYZ robotVelocity;
    POINT_XYZ robotPosition; // in BLH coordinates frame
    ...
    double H[6][15]; // Observation Matrix
    // double K[15][15];
    double P[15][15];  // posteriori covariance
    double X_b[15];

    double R[6][6];

    POINT_XYZ positionDR;
    POINT_XYZ attitudeDR;

    POINT_XYZ deltaVel;
    POINT_XYZ wBib;
    POINT_XYZ wBnb;

    POINT_XYZ deltaArmPos;
    POINT_XYZ deltaArmVel;
...
double modulusValue(POINT_XYZ data)
{
...
Array_2D transposeMatrix(Array_2D rawData)
{
    Array_2D result;
    result.x = rawData.y;
    result.y = rawData.x;
    ...
    {
        for(int j = 0; j < rawData.y; j++)
        {
            result.data[j][i] = rawData.data[i][j];
        }
    }
    return result;
}

Array_2D multiplyMaxtrix(Array_2D maxtrixA, Array_2D maxtrixB)
{
    Array_2D result;
    result.x = maxtrixA.x;
    result.y = maxtrixB.y;
    ...
    for(int i = 0; i < result.x; i++)
    {
        for(int j = 0; j < result.y; j++)
        {
            ...
            for(int k = 0; k < num; k++)
            {
                result.data[i][j] += maxtrixA.data[i][k] * maxtrixB.data[k][j];
            }
        }
    }
...
Array_2D inverseMatrix(Array_2D rawData)
{
    Array_2D result;
    result.x = rawData.x;
    result.y = rawData.y;
